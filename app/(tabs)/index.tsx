...
const checkAndRescheduleNotification = async (token: string | null) => {
    ...
    } catch (error) { console.error("Failed to check and reschedule notification:", error); }
};
...
const MainScreen: React.FC = () => {
  ...
  // (handleSubmitAnswer는 답변 후 fetchQuestion(true) -> fetchQuestion(false)로 변경하여, 답변 후엔 기존 질문이 유지되도록 함)
   const handleSubmitAnswer = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}${API_ENDPOINTS.SAVE_ANSWER}`, { questionId: currentQuestion.questionId,   answerText: userAnswer, // ✅ 이름 수정!
  emotionTag: null }, { headers: { Authorization: `Bearer ${token}` } });
      Alert.alert('기록 완료!', '네 이야기가 기록되었어.');
      setUserAnswer('');
      setIsModalVisible(false);
      // 답변 후에는 질문을 새로고침하지 않고 현재 상태를 유지합니다.
      // 다음 정기 질문은 다음 날 시간이 되어야 받을 수 있습니다.
      await handleOnDemandQuestion();
