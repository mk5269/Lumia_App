...
import { schedulePushNotification } from '../notifications'; // 알림 함수 import
...
const checkAndRescheduleNotification = async (token: string | null) => {
    ...
    try {
        ...
        if (!timestampStr) return; // 예약된 알림 없음
...
const MainScreen: React.FC = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      if (token) {
        fetchQuestion();
        checkAndRescheduleNotification(token); // 화면에 들어올 때마다 재예약 로직 실행
      }
    }, [fetchQuestion, token])
  );
